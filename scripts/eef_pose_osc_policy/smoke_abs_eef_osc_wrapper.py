"""
Smoke-test robomimic normalization and EnvRobosuite stepping for abs EEF OSC.

This does not train a policy. It builds action min-max stats from the dataset,
feeds a normalized expert action through RolloutPolicy, checks that the output
is unnormalized back to the raw absolute pose action, and steps the robomimic
environment wrapper with that action.
...
def validate_controller_metadata(env_meta):
    ...
    for part_name, body_part in body_parts.items():
        ...
        assert body_part["input_type"] == "absolute", (part_name, body_part["input_type"])
...
def main():
    ...
    pos_err_cm = float(np.linalg.norm(next_obs["robot0_eef_pos"] - raw_action[:3]) * 100)
    action_exceeds_unit = bool(np.max(np.abs(raw_action)) > 1.0)
    print("SMOKE PASSED")
    ...
    print(f"controller=OSC_POSE absolute world kp=500")
    print(f"env_action_dimension={env.action_dimension}")
    print(f"raw_action_max_abs={np.max(np.abs(raw_action)):.4f} exceeds_unit={action_exceeds_unit}")
    print(f"first_step_pos_err_cm={pos_err_cm:.3f}")
    print("reset_to_controller_refresh=covered_by_EnvRobosuite.reset_to")
    print("action_clipping=wrapper_passes_unnormalized_action_to_env.step")
    env.env.close()
